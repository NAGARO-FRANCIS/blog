...
def home(request):
    annonces = ColocationAnnonce.objects.all()[:5]
    return render(request, 'acceuil.html', {'annonces': annonces})
...
def liste_annonces(request):
    ...
    if form.is_valid():
        ...
        profil_recherche = form.cleaned_data.get('profil_recherche')

        if q:
            annonces = annonces.filter(
                Q(description__icontains=q)
                | Q(infos_logement__icontains=q)
                | Q(profil_recherche__icontains=q)
            )
        if ville:
            annonces = annonces.filter(ville__icontains=ville)
        if budget_max is not None:
            annonces = annonces.filter(budget_mensuel__lte=budget_max)
        if profil_recherche:
            annonces = annonces.filter(profil_recherche=profil_recherche)
